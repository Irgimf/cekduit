...
<x-mobile-auth-layout>
    <div style="min-height:100vh;display:flex;flex-direction:column;background:linear-gradient(160deg,#014BAA 0%,#0166E8 32%,#F0F4F8 32%);">
        <div style="padding:48px 32px 24px;text-align:center;">
            ...
            <div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:4px;">Kode OTP dikirim ke email kamu</div>
        </div>
        ...
        <div style="flex:1;background:#F0F4F8;border-radius:28px 28px 0 0;padding:28px 24px 40px;">
            ...
            <form method="POST" action="{{ route('verification.verify') }}" style="display:flex;flex-direction:column;gap:16px;">
                @csrf
                <div>
                    <label class="mobile-label" style="text-align:center;display:block;margin-bottom:10px;">Masukkan Kode OTP</label>
                ...
                <button type="submit" class="mobile-btn mobile-btn-primary">Verifikasi Email</button>
            </form>
            ...
            <div x-data="{
                ...
                <form method="POST" action="{{ route('verification.send') }}">
                    @csrf
                    <button type="submit" :disabled="countdown > 0" class="mobile-btn mobile-btn-white">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" style="width:100%;background:none;border:none;color:#94A3B8;font-size:13px;padding:8px;cursor:pointer;">
