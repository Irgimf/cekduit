<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-xl text-gray-900">Edit Pemasukan</h2>
    </x-slot>
            ...
            <div class="nb-card p-6">
                <form action="{{ route('incomes.update', $income) }}" method="POST" class="space-y-4">
                        ...
                        <select name="account_id" class="nb-input w-full px-3 py-2 text-sm">
                            @foreach ($accounts as $account)
                                <option value="{{ $account->id }}"
                                        @selected(old('account_id', $income->account_id) == $account->id)>
                                    {{ $account->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit"
                                class="nb-btn nb-btn-green px-4 py-2 text-sm">Update</button>
                    </div>
                </form>
            </div>
        </div>
