    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;">
        ...
        <a href="{{ route('accounts.create') }}"
           style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;
                  min-height:240px;border-radius:12px;border:2px dashed var(--border);
                  background:var(--white);text-decoration:none;color:var(--muted);
                  transition:all 0.2s;cursor:pointer;"
           onmouseover="this.style.borderColor='var(--blue)';this.style.background='var(--blue-light)';this.style.color='var(--blue)'"
           onmouseout="this.style.borderColor='var(--border)';this.style.background='var(--white)';this.style.color='var(--muted)'">
            <div style="width:48px;height:48px;border-radius:50%;border:2px dashed currentColor;display:flex;align-items:center;justify-content:center;">
                <svg xmlns="http://www.w3.org/2000/svg" style="width:24px;height:24px;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
            </div>
            <div style="text-align:center;">
                <p style="font-size:14px;font-weight:600;">Tambah Rekening</p>
                <p style="font-size:12px;margin-top:2px;">Dompet, Bank, atau E-Wallet</p>
            </div>
        </a>
        ...
        @foreach ($accounts as $account)
            <div class="account-card">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;">
                    <div>
                        <p style="font-size:12px;opacity:0.7;font-weight:500;margin-bottom:2px;">{{ $account->type_label }}</p>
                ...
                @if ($account->account_number)
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;">
                        <p style="font-size:13px;opacity:0.85;letter-spacing:1px;">{{ $account->account_number }}</p>
                        <button data-number="{{ $account->account_number }}"
